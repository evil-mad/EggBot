...
near unsigned char BootState;
near unsigned char ErasePageTracker;
near signed char BufferedDataIndex;
near unsigned short long ProgrammedPointer;
near unsigned char ProgrammingBuffer[BufferSize];
		...
		switch(PacketFromPC.Command)
		{
			...
			case PROGRAM_DEVICE:
			{
				...
				if(ProgrammedPointer == (unsigned short long)PacketFromPC.Address)
				{
					...
					{
						// 06/29/2010 : BPS
						// Check to see if we are being asked to program a range that is less than 
						// StartPageToErase or greater than MaxPageToErase. If so, don't save the byte but
						// just ignore it.
						if (
							(ProgrammedPointer >= (StartPageToErase * ErasePageSize))
							&&
							(ProgrammedPointer < ((MaxPageToErase + 1) * ErasePageSize))
						)
						{
...
// On entry:
//	BufferdDataIndex = number of bytes to write
//	ProgrammedPointer = pointer in FLASH to byte 1 PAST the end of where you want written (?!?)
//	ProgrammingBuffer[] = 64 byte buffer of bytes to write
void WriteFlashSubBlock(void)		//Use word writes to write code chunks less than a full 64 byte block size.
{
	...
	while(BufferedDataIndex > 0)		//While data is still in the buffer.
	{
		...
		// Only decriment it if we haven't reached zero yet!!!
		if (BufferedDataIndex)
		{
			BufferedDataIndex--;	//Used up one word from the buffer.
		}
	}
